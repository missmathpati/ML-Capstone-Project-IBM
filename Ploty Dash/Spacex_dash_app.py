...
def get_pie_chart(entered_site):
    ...
    if entered_site == 'ALL':
        # If all sites are selected, calculate total success launches count for each site
        success_by_site = spacex_df.groupby('Launch Site')['class'].sum().reset_index()
        fig = px.pie(success_by_site, values='class', names='Launch Site', title='Success Launches by Site')
...
def update_scatter_chart(selected_site, payload_range):
    if selected_site == 'ALL':
        ...
        fig = px.scatter(filtered_df, x='Payload Mass (kg)', y='class', 
                         ...
                         title='Payload Success Correlation')
    else:
